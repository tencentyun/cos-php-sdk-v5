...
class CommandToRequestTransformer {
    ...
    public function bucketStyleTransformer( CommandInterface $command, RequestInterface $request ) {
        ...
        if ($action == 'ListBuckets') {
            $uri =  "service.cos.tencentcos.cn";

            if ($this->config['endUri'] != null) {
                $uri = "service.cos." . $this->config['endUri'];
            }
            ...
            if ($this->config['enableOldDomain']) {
        ...
        if ($this->config['endUri'] == null) {
            if ($this->config['enableOldDomain']) {
                $this->config['endUri'] = "myqcloud.com";
            } else {
                $this->config['endUri'] = "tencentcos.cn";
            }
        }

        $domain_type = '.cos-internal.';

        if (!$this->config['enableInternalDomain']) {
            $domain_type = '.cos.';
        }
        ...
        $origin_host = $this->config['allow_accelerate'] ?
            $bucketname . $domain_type . 'accelerate' . '.' . $this->config['endUri'] :
            $bucketname . $this->config['endpoint'];
    ...
    public function cosDomain2CiTransformer(CommandInterface $command, $request) {
        ...
        if (key_exists($action, $ciActions)) {
            ...
            $command['Bucket'] = $bucketname;
            $domain_type = '.ci.';
            $origin_host = $bucketname . $domain_type . $this->config['region'] . ".myqcloud.com";
            $host = $origin_host;
            if ( $this->config['ip'] != null ) {
                $host = $this->config['ip'];
                if ( $this->config['port'] != null ) {
